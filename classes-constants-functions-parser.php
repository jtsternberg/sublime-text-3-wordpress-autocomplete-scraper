...
function tokenizeme() {
	...
	foreach ( $functions as $key => $function ) {
		...
		if ( ! isset( $_GET['classes_constants'] ) ) {
			$triggers[] = '{"trigger": "' . $function[0] . '", "contents": "' . $function[0] . '(' . $args . ')" },';
		}
	}
	...
	if ( isset( $_GET['classes_constants'] ) ) {
		...
		$allconstants = array_unique( $allconstants );
		$allclasses = array_unique( $allclasses );
		$total = count( $allconstants ) + count( $allclasses );
		echo "Constants/Classes (". $total . "):\n";
		echo str_replace( "\n]", ',', print_r( output_clean_json_string( $allconstants ), true ) );
		echo str_replace( "\n[\n", '', print_r( output_clean_json_string( $allclasses ), true ) );
	} else {
